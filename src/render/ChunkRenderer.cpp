...
ChunkRenderer::ChunkRenderer(TextureAtlas *atlas) : m_atlas(atlas), m_level(nullptr) {}

ChunkRenderer::~ChunkRenderer() {}

void ChunkRenderer::setLevel(Level* level) {
  m_level = level;
}
...
void ChunkRenderer::rebuildChunk(Chunk *c) {
  ...
  for (int lx = 0; lx < CHUNK_SIZE_X; lx++) {
    for (int lz = 0; lz < CHUNK_SIZE_Z; lz++) {
      for (int ly = 0; ly < CHUNK_SIZE_Y; ly++) {
        ...
        // Note: Culling optimization was removed for simplicity in this refactor, 
        // but could easily be re-added inside TileRenderer.
  ...
  if (c->trienglesCount > 0) {
    ...
    c->meshVertices = (CraftPSPVertex*)memalign(16, c->trienglesCount * sizeof(CraftPSPVertex));

    if (c->meshVertices) {
      memcpy(c->meshVertices, g_vertexBuf, c->trienglesCount * sizeof(CraftPSPVertex));
      sceKernelDcacheWritebackInvalidateRange(c->meshVertices, c->trienglesCount * sizeof(CraftPSPVertex));
    } else {
      c->trienglesCount = 0;
    }
  } else {
...
void ChunkRenderer::render(float camX, float camY, float camZ) {
  ...
  for (int cx = 0; cx < WORLD_CHUNKS_X; cx++) {
    for (int cz = 0; cz < WORLD_CHUNKS_Z; cz++) {
      ...
      ScePspFVector3 chunkPos = {
          (float)(c->cx * CHUNK_SIZE_X),
          0.0f,
